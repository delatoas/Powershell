...
function Get-StaleComputers {
    ...
    Write-Log "Searching for Windows workstations inactive since $($cutoffDate.ToString('yyyy-MM-dd'))"
    
    # Get workstation computers only (filter by OS at query level for efficiency)
    # Targets Windows 10 and Windows 11 workstations only
    ...
    Write-Log "Found $($allComputers.Count) Windows workstations in AD"
    ...
    Write-Log "Found $($staleComputers.Count) stale workstations"
...
Write-Log "  Target OS: Windows 10, Windows 11 (workstations only)"
