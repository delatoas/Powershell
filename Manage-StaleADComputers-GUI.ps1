...
function Get-StaleComputers {
    ...
    Write-GUILog "Searching for Windows workstations inactive since $($cutoffDate.ToString('yyyy-MM-dd'))"
    
    # Get workstation computers only (filter by OS at query level for efficiency)
    ...
    Write-GUILog "Found $($allComputers.Count) Windows workstations in AD"
    
    # Filter for stale workstations
    $staleComputers = $allComputers | Where-Object {
        # Double-check OS exclusion (exclude any Windows Server that slipped through)
        ...
        if ($null -eq $_.LastLogonTimestamp -or $_.LastLogonTimestamp -eq 0) {
            # Never logged on - use whenCreated
            $lastLogon = $_.whenCreated
        }
        
        return $lastLogon -lt $cutoffDate
    }
    
    Write-GUILog "Found $($staleComputers.Count) stale workstations"
...
# Load SMPA logo for title bar
$iconPath = Join-Path $PSScriptRoot "SMPA_logo_16x16.ico"
...
$mainLayout.RowStyles.Add((New-Object System.Windows.Forms.RowStyle([System.Windows.Forms.SizeType]::Absolute, 120))) | Out-Null
...
$settingsLayout.Controls.Add($lblExportPath, 2, 1)
$settingsLayout.Controls.Add($txtExportPath, 3, 1)
$settingsLayout.SetColumnSpan($txtExportPath, 2)
$settingsLayout.Controls.Add($btnBrowseExport, 5, 1)

# Button panel for row 3
...
$btnScan.Add_Click({
    ...
    try {
        ...
        Write-GUILog "Target OS: Windows 10, Windows 11 (workstations only)"
        Write-GUILog "=========================================="
...
Write-GUILog "Target: Windows 10/11 workstations only (servers excluded)"
